...
PRODUCT_NAME       := external-dns-operator
ENFORCE_LICENSE    ?=

GO_PKG   := kubeops.dev
REPO     := $(notdir $(shell pwd))
BIN      := external-dns-operator
COMPRESS ?= no
...
bin/.container-$(DOTFILE_IMAGE)-%: bin/$(BIN)-$(OS)-$(ARCH) $(DOCKERFILE_%)
	...
	@docker build --platform $(OS)/$(ARCH) --load --pull --provenance=false --sbom=false -t $(IMAGE):$(TAG_$*) -f bin/.dockerfile-$*-$(OS)_$(ARCH) .
...
uninstall:
	@cd ../installer; \
	helm uninstall external-dns-operator --namespace=$(KUBE_NAMESPACE) || true
